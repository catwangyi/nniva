...
from loss import cal_p_loss, cal_spec_loss, functional
            ...
            if torch.prod(torch.prod(X_mix_stft[i, :, :]==0))==1:
                ...
                if initial == 0:
                    ...
                    if True:
                        ...
                        p = create_p(W, label[i], alpha_iva)
                ...
                if True:
                    ...
                    loss.backward() # backward完成后，释放了中间结果，如果再次使用，会出现报错
                    # print(f'model:{model.cov.weight._version}')
...
if __name__ == "__main__":
    ...
    y_joint = auxIVA_online(mix_stft, target, dnn)
    end_time = time.time()
