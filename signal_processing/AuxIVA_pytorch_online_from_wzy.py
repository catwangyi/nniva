            ...
            if torch.prod(torch.prod(X_mix_stft[i, :, :]==0))==1:
                ...
                for k in range(K): 
                    if clean_sig == None:
                        ...
                        Y = clean_stft[i, :, :]
                        temp_sum = torch.sum(torch.abs(Y)**2)
                    r[k] = torch.sqrt(temp_sum.real)
                    if r[k] < epsi:
                        r[k] = epsi
                p = (1-alpha) / r # p就是1/r
                ...
                for k in range(K):
                    if index == 0:
                        Vinv = 1 / (V[:, :, :, k] + epsi) * temp_eye # [513, 2, 2]
                        temp_inv = Vinv[:, :, k:k+1] # [513, 2, 1]
                    else:
