...
def auxIVA_online(x, N_fft = 2048, hop_len = 0, label = None, ref_num=10, delay_num=1):
    ...
    X_mix_stft = X_mix_stft.permute(2, 1, 0) # [C, fre, time] -> [time, fre, C]
    # aux_IVA_online
    for iter in range(1):
        # init paras and buffers
        Y_all = torch.zeros_like(X_mix_stft)
        wpe_buffer = functional.pad(X_mix_stft, pad=[0, 0, 0, 0, delay_num+ref_num, 0]).permute(1, 2, 0).contiguous() # [10, 513, 2]- >[513, 2, 10]
        for i in tqdm(range(N_frame), ascii=True):
            if torch.prod(torch.prod(X_mix_stft[i, :, :]==0))==1:
                Y_all[i, :, :] = X_mix_stft[i, :, :]
            else:
                if joint_wpe:
                    temp = wpe_buffer[...,i+delay_num:i+delay_num+ref_num]
...
if __name__ == "__main__":
    ...
    delay_num = 0
    ref_num = 5
    out_path = f'audio\\vft_nara_wpe_iva_{nfft}_delay_{delay_num}_ref_{ref_num}.wav'
